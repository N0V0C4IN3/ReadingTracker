...
public sealed class ForwardingTests(GatewayFixture fixture)
{
    ...
    public async Task Sends_book_requests_to_the_catalog()
    {
        ...
        var response = await fixture.ClientFor("forwarding-reader").GetAsync("/api/books/search?isbn=9780441013593");
    ...
    public async Task Sends_library_requests_to_the_library()
    {
        ...
        var response = await fixture.ClientFor("forwarding-reader").GetAsync("/api/library");
    ...
    public async Task Carries_the_path_and_query_string_through_untouched()
    {
        ...
        await fixture.ClientFor("forwarding-reader").GetAsync("/api/books/search?title=Dune&author=Herbert");
    ...
    public async Task Carries_the_method_and_body_through_untouched()
    {
        ...
        await fixture.ClientFor("forwarding-reader").PostAsJsonAsync("/api/library", new { bookId });
    ...
    {
        var seen = fixture.ResetWithOkResponses();
        var client = fixture.ClientFor("header-reader");
        client.DefaultRequestHeaders.Add("X-Invented-Later", "still arrives");

        await client.GetAsync("/api/library");

        // Headers the caller sends are the caller's business and arrive untouched. The one
        // exception is the reader header, which the Gateway now owns outright — see
        // ReaderIdentityTests.
        Assert.Equal("still arrives", seen.Single().Headers.GetValues("X-Invented-Later").Single());
    }
    ...
    public async Task Tells_the_service_who_the_request_really_came_from()
    {
        ...
        await fixture.ClientFor("forwarding-reader").GetAsync("/api/library");
    ...
    public async Task Reaches_endpoints_that_did_not_exist_when_the_gateway_was_written()
    {
        ...
        await fixture.ClientFor("forwarding-reader").PutAsync("/api/library/anything/invented-later", null);
    ...
    public async Task Tells_a_service_that_is_down_apart_from_one_that_answered_502()
    {
        ...
        var answered = await fixture.ClientFor("forwarding-reader").GetAsync("/api/library");

        fixture.Downstream.Respond = _ => throw new HttpRequestException("connection refused");
        var unreachable = await fixture.ClientFor("forwarding-reader").GetAsync("/api/library");
    ...
    public async Task Does_not_forward_a_path_that_belongs_to_no_service()
    {
        ...
        var response = await fixture.ClientFor("forwarding-reader").GetAsync("/api/nonsense");
