...
public sealed class ReadingStatusTests(LibraryApiFixture fixture)
{
    ...
    {
        var client = ClientFor("status-reader");
        var entryId = await AddBookAsync(client);
    ...
    {
        var client = ClientFor("filter-reader");
        var reading = await AddBookAsync(client);
        var wanted = await AddBookAsync(client);
    ...
    {
        var client = ClientFor("announce-reader");
        var entryId = await AddBookAsync(client);
    ...
    {
        var mine = await AddBookAsync(ClientFor("owner-reader"));

        var response = await ClientFor("intruder-reader")
    ...
    {
        var client = ClientFor("badstatus-reader");
        var entryId = await AddBookAsync(client);
    ...
    private async Task<Guid> AddBookAsync(HttpClient client)
    {
        var bookId = Guid.NewGuid();
        fixture.Catalog.Respond = _ => StubHttpMessageHandler.Json($$"""
            { "id": "{{bookId}}", "title": "A Book", "authors": ["A. Writer"], "totalPages": 200 }
            """);
